...
async function gql<T = unknown>(query: string, variables?: Record<string, unknown>): Promise<{ data?: T; errors?: Array<{ message: string }> }> {
...
await gql(`mutation Verify($p: String!, $c: String!) { verifyOtp(phone: $p, code: $c) { home } }`, { p: phone, c: otp.data.requestOtp.devCode });

// browse
const browse = await gql<{ browseBranches: Array<{ distanceM: number; branch: { id: string; restaurant: { slug: string } } }> }>(
...
assert(!hits.some((h) => h.branch.restaurant.slug === "lajawab-bites"), "pending restaurant excluded");

// menu detail
const menuQ = await gql<{ branchBySlug: { id: string; activeMenu: { categories: Array<{ items: Array<{ id: string; name: string; modifierGroups: Array<{ name: string; minSelect: number; options: Array<{ id: string; name: string }> }> }> }> } } }>(
...
const q1 = await gql<{ quoteCart: { meetsMinimum: boolean } }>(QUOTE, {
  input: { branchId, deliveryLat: 33.5251, deliveryLng: 73.0952, lines: [{ menuItemId: naan.id, qty: 1 }] },
});
assert(q1.data!.quoteCart.meetsMinimum === false, "below-minimum flagged");

// missing required modifier
const q2 = await gql(QUOTE, {
  input: { branchId, deliveryLat: 33.5251, deliveryLng: 73.0952, lines: [{ menuItemId: biryani.id, qty: 2 }] },
});
...
const q3 = await gql<{ quoteCart: { inRadius: boolean; distanceM: number } }>(QUOTE, {
  input: { branchId, deliveryLat: 33.9, deliveryLng: 73.5, lines: [{ menuItemId: biryani.id, qty: 2, modifierOptionIds: [spiceOptId] }] },
});
assert(q3.data!.quoteCart.inRadius === false, `out-of-radius flagged (${q3.data!.quoteCart.distanceM}m)`);

// valid quote: 2x biryani (45000) = 90000, tax 13% = 11700, delivery 8000, platform 2000 -> 111700
const q4 = await gql<{ quoteCart: { subtotalMinor: number; taxTotalMinor: number; grandTotalMinor: number; meetsMinimum: boolean; inRadius: boolean } }>(QUOTE, {
  input: { branchId, deliveryLat: 33.5251, deliveryLng: 73.0952, lines: [{ menuItemId: biryani.id, qty: 2, modifierOptionIds: [spiceOptId] }] },
});
const q = q4.data!.quoteCart;
assert(q.subtotalMinor === 90_000 && q.taxTotalMinor === 11_700 && q.grandTotalMinor === 111_700, `money math (got ${q.subtotalMinor}/${q.taxTotalMinor}/${q.grandTotalMinor})`);
...
const input = {
  ...
  addressText: "House 12, Street 4, Phase 8", contactPhone: phone,
  paymentMode: "cod",
...
assert(p1.data?.placeOrder.status === "pending_acceptance", `order placed (${p1.data?.placeOrder.id})`);
assert(p1.data?.placeOrder.id === p2.data?.placeOrder.id, "double-submit returns same order id");

// out-of-radius placement rejected
const p3 = await gql(PLACE, { input: { ...input, deliveryLat: 33.9, deliveryLng: 73.5 }, key: crypto.randomUUID() });
...
const my = await gql<{ myOrders: Array<{ id: string; status: string }> }>(`query { myOrders { id status } }`);
...
const after = await gql<{ order: { status: string; events: Array<{ toStatus: string; actorRole: string | null }> } }>(
  `query { order(id: "${p1.data!.placeOrder.id}") { status events { toStatus actorRole } } }`,
);
assert(after.data?.order.status === "auto_expired", `order auto-expired (got ${after.data?.order.status})`);
assert(after.data?.order.events.some((e) => e.toStatus === "auto_expired" && e.actorRole === "system"), "system event row written");
