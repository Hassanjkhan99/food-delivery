...
assert(
  got.ok && (await got.arrayBuffer()).byteLength === png.byteLength,
  "public GET serves the bytes",
);

// size cap
const tooBig = await owner(
  `mutation { presignUpload(contentType: "image/png", byteSize: ${11 * 1024 * 1024}, kind: "image") { assetId } }`,
);
...
await owner(
  `mutation { finalizeUpload(assetId: "${preCsv.data!.presignUpload.assetId}") { status } }`,
);
const preview = await owner<{
  previewMenuCsv: Array<{ name: string; priceMinor: number; error: string | null }>;
}>(
...
assert(
  imp.data?.importMenuCsvToDraft.created === 2,
  `2 items imported into draft (${JSON.stringify(imp.data?.importMenuCsvToDraft)})`,
);

// 4) theme update visible on the public branch query
const th = await owner(
  `mutation { updateTheme(restaurantId: "${kbhId}", primaryColor: "#0e7490", cardStyle: "glass", heroEffect: "depth") { id } }`,
);
assert(!th.errors, "theme updated");
const pub = await owner<{
  branchBySlug: { restaurant: { theme: { primaryColor: string; cardStyle: string } } };
}>(
  `query { branchBySlug(slug: "karachi-biryani-house") { restaurant { theme { primaryColor cardStyle } } } }`,
);
assert(
  pub.data?.branchBySlug.restaurant.theme.primaryColor === "#0e7490" &&
    pub.data.branchBySlug.restaurant.theme.cardStyle === "glass",
  "public theme reflects change",
);
const badColor = await owner(
  `mutation { updateTheme(restaurantId: "${kbhId}", primaryColor: "teal") { id } }`,
);
assert(Boolean(badColor.errors?.length), "non-hex color rejected");

// restore the seed theme
await owner(
  `mutation { updateTheme(restaurantId: "${kbhId}", primaryColor: "#b45309", cardStyle: "tilt3d", heroEffect: "parallax") { id } }`,
);

// 5) layout update
const lay = await owner(
  `mutation { updateMenuLayout(branchId: "${branchId}", layoutJson: { displayModes: { Desserts: "grid" } }) { id } }`,
);
...
for (const o of delivered) {
  const r = await cust<{ rateOrder: { stars: number } }>(
    `mutation { rateOrder(orderId: "${o.id}", stars: 5, comment: "great") { stars } }`,
  );
