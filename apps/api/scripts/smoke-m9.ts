...
// stats
const stats = await admin<{ dashboardStats: { ordersToday: number; pendingApprovals: number; pendingRefunds: number } }>(
  `query { dashboardStats { ordersToday pendingApprovals pendingRefunds acceptanceSlaPct } }`,
);
assert(typeof stats.data?.dashboardStats.ordersToday === "number", `stats readable (${JSON.stringify(stats.data?.dashboardStats)})`);
assert((stats.data?.dashboardStats.pendingApprovals ?? 0) >= 1, "pending approval visible (Lajawab Bites)");
...
const approved = await admin<{ allRestaurants: Array<{ slug: string; status: string }> }>(`query { allRestaurants { slug status } }`);
assert(approved.data?.allRestaurants.find((r) => r.slug === "lajawab-bites")?.status === "approved", "restaurant approved");
...
const gbFull = await admin<{ allRestaurants: Array<{ id: string; slug: string }> }>(`query { allRestaurants { id slug } }`);
...
const gbMenu = await cust<{ branchBySlug: { id: string; activeMenu: { categories: Array<{ items: Array<{ id: string; name: string }> }> } } }>(
  `query { branchBySlug(slug: "green-bowl") { id activeMenu { categories { items { id name } } } } }`,
);
const bowl = gbMenu.data!.branchBySlug.activeMenu.categories.flatMap((c) => c.items).find((i) => i.name === "Steak Bowl")!;
const q = await cust<{ quoteCart: { platformFeeMinor: number; subtotalMinor: number } }>(
  ...
  { input: { branchId: gbMenu.data!.branchBySlug.id, deliveryLat: 33.5312, deliveryLng: 73.0871, lines: [{ menuItemId: bowl.id, qty: 1 }] } },
);
assert(q.data?.quoteCart.platformFeeMinor === 3_000, `chain platform fee applied after tier change (${q.data?.quoteCart.platformFeeMinor})`);
...
const rq = await admin<{ refundQueue: Array<{ id: string; amountMinor: number; destination: string; order: { code: string } }> }>(
  `query { refundQueue { id amountMinor destination order { code } } }`,
);
assert((rq.data?.refundQueue.length ?? 0) >= 1, `refund queue has ${rq.data?.refundQueue.length} case(s)`);
...
assert(decided.data?.decideRefund.status === "refunded", `refund ${refund.order.code} approved -> refunded`);
const again = await admin(`mutation { decideRefund(id: "${refund.id}", approve: true) { status } }`);
...
const anyDelivered = await admin<{ auditLogs: Array<{ action: string }> }>(`query { auditLogs(take: 10) { action } }`);
assert(anyDelivered.data?.auditLogs.some((a) => a.action === "refund.approve"), "refund decision audited");

// payout batch zeroes positive balances
const before = await admin<{ payoutCandidates: Array<{ restaurantId: string; balanceMinor: number; name: string }> }>(
...
assert((batch.data?.runPayoutBatch.length ?? 0) === positive.length, `payout batch paid ${batch.data?.runPayoutBatch.length}/${positive.length} positive balances`);
const after = await admin<{ payoutCandidates: Array<{ balanceMinor: number }> }>(`query { payoutCandidates { balanceMinor } }`);
assert(!after.data?.payoutCandidates.some((c) => c.balanceMinor > 0), "all positive balances zeroed");

// fee versioning
await admin(`mutation { updateFeeConfig(smallBusinessCommissionBps: 100, smallBusinessPlatformFeeMinor: 2000, chainCommissionBps: 800, chainPlatformFeeMinor: 3000) { id } }`);
const fees = await admin<{ currentFeeConfig: { smallBusinessCommissionBps: number } }>(`query { currentFeeConfig { smallBusinessCommissionBps } }`);
assert(fees.data?.currentFeeConfig.smallBusinessCommissionBps === 100, "new fee version active");
await admin(`mutation { updateFeeConfig(smallBusinessCommissionBps: 0, smallBusinessPlatformFeeMinor: 2000, chainCommissionBps: 800, chainPlatformFeeMinor: 3000) { id } }`);
