...
const menuQ = await customer<{ branchBySlug: { id: string; activeMenu: { categories: Array<{ items: Array<{ id: string; name: string; modifierGroups: Array<{ name: string; options: Array<{ id: string }> }> }> }> } } }>(
...
const placed = await customer<{ placeOrder: { id: string; code: string; grandTotalMinor: number } }>(
...
const riders = await owner<{ branchRiders: Array<{ id: string; user: { phone: string } }> }>(
  `query R($b: String!) { branchRiders(branchId: $b) { id user { phone } } }`, { b: branchId },
);
...
const profile = await rider<{ myRiderProfile: { isOnline: boolean } }>(`query { myRiderProfile { isOnline } }`);
assert(profile.data?.myRiderProfile.isOnline === true, "rider online");

const jobs = await rider<{ myJobs: Array<{ id: string; status: string; codAmountMinor: number; order: { id: string } }> }>(
...
const early = await rider(`mutation { riderDelivered(taskId: "${job!.id}", codCollectedMinor: ${grand}) { status } }`);
...
const picked = await rider<{ riderPickedUp: { status: string } }>(`mutation { riderPickedUp(taskId: "${job!.id}") { status } }`);
assert(picked.data?.riderPickedUp.status === "picked_up", "picked up");

const midway = await customer<{ order: { status: string } }>(`query { order(id: "${orderId}") { status } }`);
...
const final = await customer<{ order: { status: string } }>(`query { order(id: "${orderId}") { status } }`);
...
assert((earnings.data?.myEarnings.deliveredCount ?? 0) >= 1, `earnings visible (${earnings.data?.myEarnings.deliveredCount} delivered)`);
