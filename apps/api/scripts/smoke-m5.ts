...
const good = await gql<{ addPaymentMethod: { id: string; brand: string; last4: string; isDefault: boolean } }>(ADD, {
  card: { number: "4242424242424242", expMonth: 12, expYear: 2030, cvc: "123" },
});
assert(good.data?.addPaymentMethod.brand === "visa" && good.data.addPaymentMethod.last4 === "4242", "card tokenized (visa •••• 4242)");
...
const badExpiry = await gql(ADD, { card: { number: "4242424242424242", expMonth: 1, expYear: 2024, cvc: "123" } });
assert(badExpiry.errors?.[0]?.message.includes("expired"), "expired card rejected");

// fetch a menu item
const menuQ = await gql<{ branchBySlug: { id: string; activeMenu: { categories: Array<{ items: Array<{ id: string; name: string; modifierGroups: Array<{ name: string; options: Array<{ id: string }> }> }> }> } } }>(
...
const baseInput = {
  branchId, deliveryLat: 33.5104, deliveryLng: 73.1152,
  addressText: "Plaza 9, Business Bay, Phase 8", contactPhone: phone,
  lines: [{ menuItemId: burger.id, qty: 2 }],
};

// card order (chain restaurant: 8% commission + Rs30 platform fee)
const p1 = await gql<{ placeOrder: { id: string; code: string; status: string; grandTotalMinor: number } }>(PLACE, {
  input: { ...baseInput, paymentMode: "card", paymentMethodId: good.data!.addPaymentMethod.id },
  key: crypto.randomUUID(),
});
assert(p1.data?.placeOrder.status === "pending_acceptance", `card order placed ${p1.data?.placeOrder.code}`);
...
const c = await gql<{ cancelOrder: { status: string } }>(`mutation { cancelOrder(id: "${orderId}", reason: "changed my mind") { status } }`);
...
const detail = await gql<{ order: { events: Array<{ toStatus: string }> } }>(`query { order(id: "${orderId}") { events { toStatus } } }`);
assert(detail.data?.order.events.some((e) => e.toStatus === "cancelled"), "cancel event recorded");
