...
assert(
  board.data?.boardOrders.some((o) => o.id === orderId && o.status === "pending_acceptance"),
  "order visible on board as NEW",
);
...
assert(
  accepted.data?.acceptOrder.prepEtaMinutes === 20,
  `accepted with ETA (status ${accepted.data?.acceptOrder.status})`,
);

const reaccept = await owner(
  `mutation { acceptOrder(id: "${orderId}", prepEtaMinutes: 10) { status } }`,
);
assert(Boolean(reaccept.errors?.length), "double-accept rejected");

await owner(`mutation { startPreparing(id: "${orderId}") { status } }`);
const ready = await owner<{ markReady: { status: string } }>(
  `mutation { markReady(id: "${orderId}") { status } }`,
);
...
const riders = await owner<{ branchRiders: Array<{ id: string; user: { name: string } }> }>(
  `query R($b: String!) { branchRiders(branchId: $b) { id user { name } } }`,
  { b: branchId },
);
...
assert(
  assigned.data?.assignRider.status === "rider_assigned",
  `rider assigned (${rider.user.name})`,
);
...
assert(
  (track.data?.order.events.length ?? 0) >= 5,
  `full event chain (${track.data?.order.events.length} events)`,
);

// menu draft -> publish
const draft = await owner<{
  draftMenu: { id: string; version: number; categories: Array<{ id: string; name: string }> };
}>(`query D($b: String!) { draftMenu(branchId: $b) { id version categories { id name } } }`, {
  b: branchId,
});
...
const before = await customer<{
  branchBySlug: {
    activeMenu: { version: number; categories: Array<{ items: Array<{ name: string }> }> };
  };
}>(
...
assert(
  !before
    ...
    .some((i) => i.name === "Smoke Test Special"),
  "draft item hidden pre-publish",
);

const pub = await owner<{ publishMenu: { version: number } }>(
  `mutation { publishMenu(branchId: "${branchId}") { version } }`,
);
assert(
  (pub.data?.publishMenu.version ?? 0) > liveVersion,
  `published v${pub.data?.publishMenu.version}`,
);
...
assert(
  after
    .data!.branchBySlug.activeMenu.categories.flatMap((c) => c.items)
    .some((i) => i.name === "Smoke Test Special"),
  "published item visible to customers",
);

// wallet
const kbhId = (
  await owner<{ myRestaurants: Array<{ id: string; slug: string }> }>(
    `query { myRestaurants { id slug } }`,
  )
).data!.myRestaurants.find((r) => r.slug === "karachi-biryani-house")!.id;
const wallet = await owner<{
  walletBalance: number;
  walletStatement: Array<{ memo: string }>;
  payoutHistory: Array<{ status: string }>;
}>(
  `query W($r: String!) { walletBalance(restaurantId: $r) walletStatement(restaurantId: $r) { memo } payoutHistory(restaurantId: $r) { status } }`,
  { r: kbhId },
);
assert(
