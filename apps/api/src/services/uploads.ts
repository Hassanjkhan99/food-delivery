...
export async function presignUpload(
  ...
  if (!rules.mimes.includes(parsed.contentType as never)) {
    throw new GraphQLError(`Content type ${parsed.contentType} not allowed for ${parsed.kind}`);
  }
  if (parsed.byteSize > rules.maxBytes) {
    throw new GraphQLError(`File exceeds the ${Math.round(rules.maxBytes / 1024 / 1024)} MB limit`);
  }
...
export async function finalizeUpload(userId: string, assetId: string) {
  ...
  if (!asset || asset.ownerId !== userId) throw new GraphQLError("Asset not found");
  if (asset.status === "finalized") return asset;

  const head = await localDiskStore.head(asset.objectKey);
  if (!head.exists) throw new GraphQLError("Upload not received yet");
