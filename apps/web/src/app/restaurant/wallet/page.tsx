    ...
    <main className="max-w-2xl">
      ...
      <div className="mb-6 rounded-2xl border border-neutral-200 bg-white p-6">
        ...
        {balance < 0 && (
          <p className="mt-1 text-xs text-red-500">
            Negative balance: platform fees from COD orders exceed card-order earnings.
            It nets against future card orders or is invoiced.
          </p>
        )}
      </div>
      ...
      <div className="mb-6 space-y-2">
        {data?.payoutHistory.map((p) => (
          <div key={p.id} className="flex items-center justify-between rounded-xl border border-neutral-200 bg-white p-3 text-sm">
            <span>
              {p.reference ?? p.id.slice(0, 8)}
              {p.paidAt && <span className="ml-2 text-xs text-neutral-400">{new Date(p.paidAt as unknown as string).toLocaleDateString()}</span>}
            </span>
        ...
        {data?.payoutHistory.length === 0 && <p className="text-sm text-neutral-400">No payouts yet.</p>}
      </div>
      ...
      <div className="space-y-1">
        {data?.walletStatement.map((e) => (
          <div key={e.id} className="flex items-center justify-between rounded-lg bg-white px-3 py-2 text-sm">
            ...
            <span className={`ml-3 shrink-0 font-mono ${e.creditMinor > 0 ? "text-green-700" : "text-red-600"}`}>
              {e.creditMinor > 0 ? `+${formatRs(e.creditMinor)}` : `−${formatRs(e.debitMinor)}`}
            </span>
          </div>
        ))}
        {data?.walletStatement.length === 0 && <p className="text-sm text-neutral-400">No entries yet.</p>}
      </div>
    </main>
  );
}
