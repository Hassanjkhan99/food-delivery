...
export function ItemModal({
  ...
  function submit(clearFirst = false) {
    ...
    // deep-link (?item=). Editing an existing line is still allowed. (#03 + #39)
    if (!orderable && !edit) return;
      ...
      >
        ...
        <div className="border-t border-kd-border bg-kd-surface px-6 py-4">
          <div className="flex items-center justify-between gap-3">
            ...
            <Button
              className="flex-1"
              // Closed/paused branches disable Add (but Update on an existing line is allowed).
              disabled={!orderable && !edit}
              onClick={() => submit()}
              aria-invalid={!valid}
              title={!orderable && !edit ? disabledLabel : undefined}
            >
              {!orderable && !edit
